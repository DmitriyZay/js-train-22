// Мементо (Memento) — це патерн програмування, який забезпечує збереження 
//стану об'єкта для подальшого відновлення
...
class Version {
 ...
 static #versions=[];
  // Статичний метод create приймає аргумент content (текст версії)
  // і створює новий екземпляр класу Version в який передає content .
...
console.log('поточний текст', writer.content);

// Відновлюємо попередній текст
writer.restore();
console.log('попередній текст', writer.content);

// Ще раз відновлюємо попередній текст
writer.restore();
console.log('відновлюємо попередній текст',writer.content);
